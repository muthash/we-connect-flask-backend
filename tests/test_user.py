...
class TestDeleteAccount(BaseTestCase):
    ...
    def test_incorrect_password(self):
        """Test incorrect password while deleting account"""
        self.get_login_token()
        res = self.make_request('/api/v1/delete-account', data=dict(password='test123'), method='delete')
        result = json.loads(res.data.decode())
        self.assertEqual(result['message'], "Invalid password")
    ...
    def test_invalid_password(self):
        """Test incorrect password while deleting account"""
        self.get_login_token()
        res = self.make_request('/api/v1/delete-account', data=dict(password=''), method='delete')
        result = json.loads(res.data.decode())
        self.assertEqual(result['message'], ['Please enter your password'])
        self.assertEqual(res.status_code, 400)

    def test_json_request(self):
        """Test delete account request for valid json"""
        self.register_user()
        login_res = self.login_user()
        result = json.loads(login_res.data.decode())
        self.header['Authorization'] = 'Bearer ' + result['access_token']
